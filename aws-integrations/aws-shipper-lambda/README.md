...
2. [Supported Services](#supported-services)
 ...
 Created automatically from [coralogix-aws-shipper](https://github.com/coralogix/coralogix-aws-shipper) . To make a change to the template please go to the this [link](https://github.com/coralogix/coralogix-aws-shipper/blob/master/template.yaml)
...
| CloudWatchLogGroupPrefix | Prefix of the CloudWatch log groups that will trigger the lambda. If your log groups are `log-group1, log-group2, log-group3`, you can set the value to `log-group`. When using this variable, you will not be able to see the log groups as trigger for the lambda. The parameter dose not replace **CloudWatchLogGroupName** parameter. |               |                    |
